...
class CategoryModel(models.Model):
    ...
    def related_products(self):
        return ProductModel.objects.filter(category=self, is_avalible=True, discount_percent__gt=29)
...
class BrandModel(models.Model):
    ...
    name                =models.CharField(unique=True ,max_length=50)
    description         =models.TextField(blank=True, max_length=255)
    brand_image         =models.ImageField(upload_to="uploads/brnad", blank=True)
    slug                =models.SlugField(unique=True, max_length=100)
...
class ProductModel(models.Model):
    ...
    # Add the warranty field with choices
    warranty = models.CharField(
        max_length=5,  # You can adjust the max length as needed
        choices=WARRANTY_CHOICES,
        default='month'  # You can set a default value if needed
    )   
    warranty_duration = models.IntegerField(default=1)
    
    def save(self, *args, **kwargs):
        if self.category_trending:
            # If a product is marked as trending in a category, ensure no other product in the same category is marked as trending
...
class ExclusivePromotions(models.Model):
    ...
    def clean(self):
        # Check if there are already 5 products assigned to promotions
    ...
    def save(self, *args, **kwargs):
        # You can also perform additional validation here before saving if needed
        self.full_clean()
...
class ProductVariation(models.Model):
        ...
        main_title = models.CharField(max_length=100)  # E.g., "Capacity"
        title = models.CharField(max_length=100)  # E.g., "16GB"
        description = models.TextField()  # E.g., detailed description or specs
